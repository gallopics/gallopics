...
const buildAbsoluteUrl = (path: string) => new URL(`${routerBase}${path}`, window.location.origin).toString();
...
const buildClerkMetadata = (overrides?: Record<string, unknown>) => ({
    ...
    photographerProfileId: PROTOTYPE_USER.id,
    role: 'pg',
    ...overrides,
});
...
const SignInForm: React.FC<FormProps> = ({ isLoading, setIsLoading, onClose }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setErrors({
                form: getClerkErrorMessage(error, 'Unable to sign in with those credentials.'),
            });
        } finally {
            setIsLoading(false);
        }
    ...
    return (
        <div className="auth-form-wrapper">
            <div className="auth-oauth-group">
                <button className="auth-btn-oauth" type="button" onClick={() => startOAuth('oauth_google')}>
                ...
                <button className="auth-btn-oauth" type="button" onClick={() => startOAuth('oauth_apple')}>
            ...
            <form id="auth-main-form" className="auth-form" onSubmit={handleSubmit}>
                <div className="auth-input-group">
                    <label className="auth-label" htmlFor="signin-identifier">Username</label>
                    <input
...
const RegisterForm: React.FC<FormProps> = ({ isLoading, setIsLoading, onClose }) => {
    const navigate = useNavigate();
    const clerk = useClerk();
    ...
    const handleCreateAccount = async () => {
        ...
        try {
            ...
            setStep('verify');
            setErrors({
                form: 'We sent a verification code to your email. Enter it below to finish creating your account.',
            });
        } catch (error) {
            console.error('Clerk sign-up error', error);
            setErrors({
                form: getClerkErrorMessage(error, 'Unable to create your account right now.'),
            });
        } finally {
            setIsLoading(false);
        }
    ...
    const handleVerifyEmail = async () => {
        ...
        try {
            ...
            console.error('Clerk sign-up verification result', result);
            ...
            setErrors({
                form: 'Clerk still requires additional verification before the account can be activated.',
            });
        } catch (error) {
            console.error('Clerk sign-up verification error', error);
            setErrors({
                form: getClerkErrorMessage(error, 'Unable to verify your email address.'),
            });
        } finally {
            setIsLoading(false);
        }
        ...
        <div className="auth-form-wrapper">
            ...
            <form id="auth-main-form" className="auth-form" onSubmit={handleSubmit}>
                {step === 'verify' ? (
                    <>
                        <div className="auth-input-group">
                            <label className="auth-label" htmlFor="reg-code">Verification Code</label>
                            <input
                                id="reg-code"
                                type="text"
