...
export const CheckoutPanel: React.FC<CheckoutPanelProps> = ({ total, onPay }) => {
    ...
    // Shared input class
    const emailInputClass = "w-full px-[var(--spacing-md)] py-[var(--spacing-md)] border border-[var(--color-border)] rounded-[var(--radius-md)] text-base transition-all duration-200 ease-linear bg-[var(--color-surface)] outline-none focus:border-black focus:bg-white focus:shadow-[0_0_0_4px_rgba(0,0,0,0.05)] disabled:bg-[var(--color-bg)] disabled:text-[var(--color-text-secondary)] disabled:border-transparent";

    return (
        <div className="bg-white">
            <h3 className="text-[1.25rem] font-bold mb-6 text-[var(--color-text-primary)]">Checkout</h3>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-6">
                    ...
                    <div className="flex flex-col gap-4">
                        ...
                        {step === 'verify' && (
                            <div className="verify-step">
                                ...
                                {otpError ? (
                                    <p className="text-[0.75rem] text-[var(--color-danger)] mt-[6px] font-medium">{otpError}</p>
                                ) : (
                ...
                >
                    <label className="block text-[0.875rem] font-semibold mb-3">Select payment method</label>
                    ...
                    <div className="flex flex-col gap-[10px]">
                        ...
                        <label className={`relative cursor-pointer`}>
                            ...
                            <div className={`flex items-center gap-4 py-[14px] px-4 border border-[var(--color-border)] rounded-[var(--radius-md)] transition-all duration-200 ease-[cubic-bezier(0.2,0,0.2,1)] bg-white hover:border-[var(--color-border)] hover:bg-[var(--ui-bg-subtle)] ${paymentMethod === 'swish' ? 'border-black bg-[var(--color-bg)] shadow-[0_2px_8px_rgba(0,0,0,0.04)]' : ''}`}>
                                ...
                                <div className="flex-1 flex flex-col gap-0.5">
                                    <span className="text-[0.875rem] font-bold text-[var(--color-text-primary)]">Swish</span>
                                    <span className="text-[0.75rem] text-[var(--color-text-secondary)] font-normal">Fastest</span>
                                </div>
                                <div className={`w-5 h-5 rounded-full border border-[var(--color-border)] flex items-center justify-center transition-all duration-200 ${paymentMethod === 'swish' ? 'bg-black border-black text-white' : 'text-transparent'}`}>
                                    <Check size={16} />
                                </div>
                            </div>
                        </label>
                        ...
                        <label className={`relative cursor-pointer`}>
                            ...
                            <div className={`flex items-center gap-4 py-[14px] px-4 border border-[var(--color-border)] rounded-[var(--radius-md)] transition-all duration-200 ease-[cubic-bezier(0.2,0,0.2,1)] bg-white hover:border-[var(--color-border)] hover:bg-[var(--ui-bg-subtle)] ${paymentMethod === 'klarna' ? 'border-black bg-[var(--color-bg)] shadow-[0_2px_8px_rgba(0,0,0,0.04)]' : ''}`}>
                                <div className="w-10 h-10 rounded-[var(--radius-md)] bg-[#ffb3c7] text-black flex items-center justify-center flex-shrink-0">
                                    <span className="font-black text-[1.2rem] tracking-[-1px]">K.</span>
                                </div>
                                <div className="flex-1 flex flex-col gap-0.5">
                                    <span className="text-[0.875rem] font-bold text-[var(--color-text-primary)]">Klarna</span>
                                    <span className="text-[0.75rem] text-[var(--color-text-secondary)] font-normal">Pay later</span>
                                </div>
                                <div className={`w-5 h-5 rounded-full border border-[var(--color-border)] flex items-center justify-center transition-all duration-200 ${paymentMethod === 'klarna' ? 'bg-black border-black text-white' : 'text-transparent'}`}>
                                    <Check size={16} />
                                </div>
                            </div>
                        </label>
                        ...
                        <label className={`relative cursor-pointer`}>
                            ...
                            <div className={`flex items-center gap-4 py-[14px] px-4 border border-[var(--color-border)] rounded-[var(--radius-md)] transition-all duration-200 ease-[cubic-bezier(0.2,0,0.2,1)] bg-white hover:border-[var(--color-border)] hover:bg-[var(--ui-bg-subtle)] ${paymentMethod === 'card' ? 'border-black bg-[var(--color-bg)] shadow-[0_2px_8px_rgba(0,0,0,0.04)]' : ''}`}>
                                ...
                                <div className="flex-1 flex flex-col gap-0.5">
                                    <span className="text-[0.875rem] font-bold text-[var(--color-text-primary)]">Card</span>
                                    <span className="text-[0.75rem] text-[var(--color-text-secondary)] font-normal">Visa, MC</span>
                                </div>
                                <div className={`w-5 h-5 rounded-full border border-[var(--color-border)] flex items-center justify-center transition-all duration-200 ${paymentMethod === 'card' ? 'bg-black border-black text-white' : 'text-transparent'}`}>
                                    <Check size={16} />
                                </div>
                            </div>
                        </label>
                        ...
                        <label className={`relative cursor-pointer`}>
                            ...
                            <div className={`flex items-center gap-4 py-[14px] px-4 border border-[var(--color-border)] rounded-[var(--radius-md)] transition-all duration-200 ease-[cubic-bezier(0.2,0,0.2,1)] bg-white hover:border-[var(--color-border)] hover:bg-[var(--ui-bg-subtle)] ${paymentMethod === 'digital' ? 'border-black bg-[var(--color-bg)] shadow-[0_2px_8px_rgba(0,0,0,0.04)]' : ''}`}>
                                ...
                                <div className="flex-1 flex flex-col gap-0.5">
                                    <span className="text-[0.875rem] font-bold text-[var(--color-text-primary)]">Apple / Google</span>
                                    <span className="text-[0.75rem] text-[var(--color-text-secondary)] font-normal">One-tap</span>
                                </div>
                                <div className={`w-5 h-5 rounded-full border border-[var(--color-border)] flex items-center justify-center transition-all duration-200 ${paymentMethod === 'digital' ? 'bg-black border-black text-white' : 'text-transparent'}`}>
                                    <Check size={16} />
                                </div>
                            </div>
                        </label>
