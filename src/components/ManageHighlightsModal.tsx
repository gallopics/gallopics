...
export const ManageHighlightsModal: React.FC<ManageHighlightsModalProps> = ({
    ...
    const eventOptions = myEvents.map(e => ({ label: `${e.title} (${e.date})`, value: e.id }));
            ...
            >
                ...
                <div className="modal-body-standard">
                    ...
                    <div className="mb-5 flex items-center gap-3">
                        <label className="text-[var(--modal-label-size)] font-[var(--modal-label-weight)] text-[var(--color-text-secondary)] whitespace-nowrap">Select from event:</label>
                    ...
                    <div className="text-[var(--modal-label-size)] text-[var(--color-text-secondary)] mb-4 flex justify-between items-center">
                        <div>
                            Selected: <span className={selectedIds.size >= 10 ? 'text-[var(--color-danger)] font-semibold' : 'font-medium'}>{selectedIds.size} / 10</span>
                        </div>
                        {selectedIds.size > 0 && (
                            <button
                    ...
                    <div className="grid [grid-template-columns:repeat(auto-fill,minmax(140px,1fr))] gap-3">
                        {currentPhotos.length === 0 ? (
                            <div className="[grid-column:1/-1] text-center py-10 text-[var(--color-text-secondary)]">No photos in this event.</div>
                        ) : (
                            currentPhotos.map(photo => {
                                ...
                                return (
                                    <div
                                        ...
                                        style={isSelected ? { boxShadow: '0 0 0 2.5px var(--color-brand-primary)' } : undefined}
                                    ...
                                    >
                                        ...
                                        {/* Checkbox top-left */}
                                        <div className={`absolute top-2 left-2 z-10 transition-opacity duration-150 ${isSelected ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}>
                ...
                <div className="modal-footer-actions">
                    <button className="edit-profile-btn-cancel" onClick={onClose}>Cancel</button>
                    <button className="edit-profile-btn-save" onClick={handleSave}>Save changes</button>
                </div>
