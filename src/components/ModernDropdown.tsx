...
export const ModernDropdown: React.FC<ModernDropdownProps> = ({
                ...
                {filteredOptions.length > 0 ? (
                    filteredOptions.map((option, index) => (
                        <button
                            ...
                            style={option.value === value ? { backgroundColor: 'var(--brand-tint-active)', color: 'var(--color-text-primary)', fontWeight: 500 } : undefined}
                        ...
                        >
                            {/* 1. Icon Column */}
                            {hasIcons && (
                                <span className="text-[1.1em] flex items-center justify-center w-6">{option.icon}</span>
                            )}

                            {/* 2. Label Column */}
                            <div className="flex flex-col gap-0.5 text-left min-w-0">
                                <div className="text-[0.875rem] font-medium text-[var(--color-text-primary)] leading-[1.4] truncate">{option.label}</div>
                                {option.subtext && <div className="text-[0.75rem] text-[var(--color-text-secondary)] leading-[1.4]">{option.subtext}</div>}
                                {option.description && <div className="text-[0.75rem] text-[var(--color-text-tertiary)] leading-[1.5] whitespace-normal break-words mt-0.5">{option.description}</div>}
                            </div>

                            {/* 3. Check Column */}
                            {option.value === value ? (
                                <Check size={16} className="text-[var(--color-text-primary)] opacity-100" />
                            ) : (
                                <div />
                            )}
                        </button>
                    ...
                    <div className="px-4 py-3 text-center text-[0.75rem] text-[var(--color-border)]">No results</div>
                )}
            </div>
        </div>
    );
    ...
    return (
        ...
        >
            ...
            >
                {icon && (
                    <span className={`flex items-center text-[${isPill ? '1rem' : '1.1em'}] ${isPill ? 'w-4 justify-center' : 'w-5 justify-center'}`}>
                        {icon}
                    </span>
                )}
                <div className="flex-1 flex flex-col text-left gap-1 min-w-0 overflow-hidden">
                    ...
                    {selectedOption?.subtext && (
                        <span className="text-[0.75rem] text-[var(--color-text-secondary)] font-normal">{selectedOption.subtext}</span>
                    )}
                </div>
                <ChevronDown className={`self-center flex-shrink-0 text-inherit transition-transform duration-200 ease-linear ${isOpen ? 'rotate-180' : ''}`} size={16} />
            </button>
