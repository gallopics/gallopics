...
export const ModernSearchBar: React.FC<ModernSearchBarProps> = ({
    ...
    const getIconBoxClass = (type: string, isLight: boolean) => {
        if (isLight) {
            switch (type) {
                case 'event': return 'text-[#1976D2] bg-[rgba(25,118,210,0.08)]';
                case 'rider': return 'text-[#7c3aed] bg-[rgba(124,58,237,0.08)]';
                case 'horse': return 'text-[#E65100] bg-[rgba(230,81,0,0.08)]';
                default: return 'bg-[var(--ui-bg-subtle)] text-[var(--color-text-secondary)] border-black/[0.05]';
            }
        }
        switch (type) {
            case 'event': return 'text-[#90caf9] bg-[rgba(144,202,249,0.1)]';
            case 'rider': return 'text-[#a78bfa] bg-[rgba(167,139,250,0.1)]';
            case 'horse': return 'text-[#ffcc80] bg-[rgba(255,204,128,0.1)]';
            default: return 'bg-[var(--color-text-primary)] text-[var(--color-border)] border-white/[0.05]';
        }
    };
    ...
    const renderResults = () => {
            ...
            >
                {hasResults ? (
                    groupOrder.map(type => {
                        ...
                        return (
                            <div key={type} className={`pb-1 mb-1 last:border-b-0 last:mb-0 last:pb-0 ${isLight ? 'border-b border-black/[0.04]' : 'border-b border-white/[0.06]'}`}>
                                <div className={`px-4 pt-[10px] pb-1 text-[0.75rem] uppercase tracking-[0.08em] font-semibold ${isLight ? 'text-[var(--color-text-secondary)]' : 'text-[var(--color-text-secondary)]'}`}>
                                    {getGroupLabel(type)}
                                </div>
                                {groupItems.map(item => (
                                    <div
                                    ...
                                    >
                                        <div className={`w-8 h-8 rounded-full flex items-center justify-center mr-3 flex-shrink-0 border border-white/[0.05] ${getIconBoxClass(item.type, isLight)}`}>
                                            {getIcon(item.type)}
                                        </div>
                                        <div className="flex flex-col justify-center overflow-hidden flex-1">
                                            <span className={`text-[0.875rem] font-medium whitespace-nowrap overflow-hidden text-ellipsis leading-[1.2] ${isLight ? 'text-[var(--color-text-primary)]' : 'text-[#EEE]'}`}>
                                                {item.title}
                                            </span>
                                            <span className={`text-[0.75rem] whitespace-nowrap overflow-hidden text-ellipsis mt-0.5 leading-[1.2] ${isLight ? 'text-[var(--color-text-secondary)]' : 'text-[var(--color-text-secondary)]'}`}>
                                                {item.subtitle}
                                            </span>
                                        </div>
                                        <ChevronRight
                    ...
                    query.length >= 2 && <span className={`no-result-text text-[var(--color-text-secondary)]`}>No matches found</span>
                )}
            </div>
        );
        ...
        if (dropdownRect && !usePortal) {
            return (
                <div className={`modern-search-wrapper ${theme}-theme ${collapsible ? 'is-collapsible expanded' : ''} static`}>
                    {content}
                </div>
            );
        }
    ...
    const mobileOverlayContent = (
        <div
            className="fixed inset-0 z-[2000] pointer-events-none pt-[calc(env(safe-area-inset-top)+12px)] px-3 flex flex-col items-center"
        >
            <div
            ...
            <div className="relative w-full max-w-full mx-auto z-[2005] pointer-events-auto">
                <div className="bg-[#1A1A1A] border border-white/15 rounded-xl px-4 h-12 flex items-center w-full shadow-[0_4px_20px_rgba(0,0,0,0.3)]">
                    <Search className="text-[var(--color-text-secondary)] flex-shrink-0" size={20} />
                    <input
                ...
                {/* Mobile Helper Line */}
                <div
                    className="mt-2 px-0 w-full text-white/90 text-[12px] font-medium tracking-[0.02em] [text-shadow:0_1px_2px_rgba(0,0,0,0.6)] whitespace-normal leading-[1.3] line-clamp-2 text-center pointer-events-none"
                >
    ...
    const getBarClass = () => {
        if (collapsible) {
            ...
            const transition = 'transition-[width,border-radius,background-color] duration-300 ease-[cubic-bezier(0.2,0.8,0.2,1)]';
            if (!isExpanded) {
                ...
                >
                    ...
                    <input
                        ...
                        className={`${collapsible && !isExpanded ? 'flex-none' : 'flex-1'} border-none bg-transparent font-normal outline-none min-w-0 transition-[font-size,font-weight] duration-200 ease-linear not-placeholder-shown:text-base not-placeholder-shown:font-medium ${
                            collapsible && !isExpanded ? 'opacity-0 pointer-events-none w-0' : 'opacity-100 pointer-events-auto w-full'
