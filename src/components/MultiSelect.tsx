                            ...
                            >
                                <div className="flex! flex-row! items-center! justify-between! text-left flex-1 min-w-0 m-0 p-0">
                                    <div className="flex! flex-row! items-center! gap-3 min-w-0 mr-auto">
                                        {option.icon && (
                                            <img src={option.icon} alt="" className="w-6 h-6 rounded-full object-cover flex-shrink-0" />
                                        )}
                                        <div className="text-[0.875rem] font-medium whitespace-nowrap overflow-hidden text-ellipsis">{option.label}</div>
                                    </div>
                                    {option.subtext && (
                                        <div className="text-[0.75rem] text-[var(--color-border)] ml-3 whitespace-nowrap">{option.subtext}</div>
                                    )}
                                </div>
                                {isSelected && <Check size={16} className="text-[var(--color-brand-primary)] ml-2 flex-shrink-0" />}
                            </button>
                        );
                    })
                ) : (
                    <div className="p-3 text-center text-[var(--color-text-secondary)] text-[0.875rem]">No results</div>
                )}
            </div>
        </div>
    );
                ...
                <div className="flex flex-wrap gap-[6px] flex-1">
                    {selectedOptions.length > 0 ? (
                        selectedOptions.map(opt => (
                            ...
                            >
                                {opt.icon && (
                                    <img src={opt.icon} alt="" className="w-5 h-5 rounded-full object-cover -ml-1" />
                                )}
                        ...
                        <span className="text-[var(--color-text-secondary)] text-[0.875rem]">{placeholder}</span>
                    )}
                </div>
                <ChevronDown className={`text-inherit transition-transform duration-200 ease-linear ${isOpen ? 'rotate-180' : ''}`} size={16} />
            </div>
