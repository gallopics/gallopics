...
const HeroCard = ({ extraClass = '', title, description }: HeroCardProps) => (
  <div className={`hero-card ${extraClass}`}>
    <div className="flex flex-col gap-4 max-w-[480px] z-[2] flex-1">
      ...
      <p className="text-base leading-relaxed text-white/90 font-[450] m-0">
        {description}
      </p>
    </div>
      ...
      <div className="hero-carousel-container">
        ...
        >
          ...
          <div className="hero-card-secondary--pg">
            <div className="flex flex-col gap-4 max-w-[480px] z-[2] flex-1">
              <h2 className="font-['Sora',sans-serif] text-2xl max-md:text-[2.5rem] font-semibold tracking-[-0.02em] text-white leading-[1.1] m-0 [text-wrap:balance]">
                I am a <br />
                Photographer
              </h2>
              <p className="text-[0.8125rem] leading-[1.5] text-white/90 m-0">
                Gallopics is a platform for competition photographers. Book
                events, upload galleries, manage orders and track your sales –
                all in one place.
              </p>
            </div>
          ...
          <div className="hero-card-secondary">
            <div className="flex flex-col gap-4 max-w-[480px] z-[2] flex-1">
              ...
              <p className="text-[0.8125rem] leading-[1.5] text-white/90 m-0">
                Helps you find the right photographers, coordinate coverage, and
                make it easy for riders to discover and purchase their photos.
              </p>
            </div>
            <div className="mt-8 max-md:mt-auto">
              <a
...
export const TitleHeader: React.FC<TitleHeaderProps> = ({
            ...
            <div className="grid grid-cols-[1fr_280px] gap-6 items-stretch max-lg:grid-cols-[1fr_240px] max-md:grid-cols-1">
              ...
              <div className="hero-card-secondary max-md:hidden">
                <div className="flex flex-col gap-4">
                  ...
                  <p className="text-[0.875rem] leading-[1.6] text-white/80 m-0">
                    {consoleDescription}
                  </p>
                </div>
                <a
  ...
  const alignClass =
    variant === 'upload' || variant === 'workspace'
      ? 'items-center'
      : 'items-end';
  ...
  return (
    <section
      className={`title-header variant-${variant} ${compact ? 'compact' : ''} ${className}`}
    >
      <div className="container">
        <div
          className={`flex justify-between gap-8 ${alignClass} max-md:flex-col max-md:items-start max-md:gap-6`}
        >
          <div className="flex flex-col gap-3">
            <div
              className={`title-header-inner-flex flex items-center gap-6 ${avatarMobileRow ? 'max-md:flex-row max-md:items-start max-md:gap-5' : 'max-md:flex-col max-md:items-start max-md:gap-4'}`}
            >
              ...
              <div className="title-text-group max-md:w-full">
                {topSubtitle && (
                  <div className="title-header-top-subtitle">{topSubtitle}</div>
                )}
                <h1 className="title-header-title">{title}</h1>
                {subtitle && (
                  <div className="title-header-subtitle">{subtitle}</div>
                )}
                {stats && (
                  <div className="flex items-baseline gap-x-3 gap-y-1 text-[0.875rem] text-[var(--color-text-secondary)] flex-wrap leading-[1.15] mt-4">
                    {stats}
                  </div>
                )}
                {description && (
                  <div className="max-w-[600px] text-[1rem] leading-[1.4] text-[var(--color-text-secondary)] mt-4">
                    {description}
                  </div>
                )}
              </div>
          ...
          {optionalClose && (
            <div className="ml-[var(--spacing-md)] flex items-center">
              {optionalClose}
            </div>
          )}
        </div>
