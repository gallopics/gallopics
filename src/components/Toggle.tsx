...
export const Toggle: React.FC<ToggleProps> = ({ checked = false, onChange, label, disabled }) => {
    return (
        <label className={`inline-flex items-center gap-3 cursor-pointer transition-opacity duration-200 ${disabled ? 'disabled' : ''}`}>
            <div className="relative w-11 h-6 flex-shrink-0">
                ...
                <span
                    className={[
                        ...
                        checked ? 'bg-[var(--color-brand-primary)]' : 'bg-[var(--color-border)]',
