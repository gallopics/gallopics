                ...
                {!isSuccess && (
                    <div className="bg-[var(--color-surface)] pb-6 max-md:pb-5 border-b border-[var(--color-border)]">
                        <div className="container">
                            <h1 className="text-[1.75rem] max-md:text-[1.375rem] font-extrabold mb-0 text-[var(--color-text-primary)] leading-[1.2]">Cart</h1>
                        </div>
                    </div>
                )}
            </div>
                    ...
                    {isSuccess ? (
                        <div className="flex flex-col gap-[60px]">
                            <div className="flex justify-center py-[60px]">
                                <div className="bg-white rounded-[var(--radius-lg)] border border-[var(--color-border)] px-10 py-[60px] max-w-[500px] w-full text-center shadow-[0_10px_40px_rgba(0,0,0,0.05)]">
                                    ...
                                    <h1 className="text-[2rem] font-extrabold mb-4">Payment Successful!</h1>
                                    <p className="text-[var(--color-text-secondary)] leading-[1.6] mb-10">Thank you for your purchase. We've sent a delivery email with your download links to your inbox.</p>

                                    <div className="flex flex-col gap-4">
                                        <button className="py-[14px] px-8 bg-black text-white border-none rounded-[var(--radius-full)] font-semibold cursor-pointer transition-all duration-200 ease-in-out hover:bg-[var(--color-text-primary)] hover:-translate-y-px hover:shadow-[0_4px_12px_rgba(0,0,0,0.1)]" onClick={() => navigate('/')}>
                                            Continue browsing
                                        </button>
                                        <p className="text-[0.8125rem] text-[var(--color-text-secondary)] opacity-70">Download links are active for 24 hours.</p>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div className="mt-8 border-t border-black/[0.06] pt-6 hidden md:block">
                                    <div className="mb-5">
                                        <div className="flex items-center gap-2">
                                            <Eye size={20} className="text-[var(--color-text-secondary)] opacity-70 w-[18px] h-[18px]" />
                                            <h2 className="text-[0.875rem] font-semibold text-[var(--color-text-secondary)] m-0 tracking-[0.02em]">Recently viewed</h2>
                                        </div>
                                    </div>
                        ...
                        <div className="flex flex-col gap-[60px]">
                            <div className="text-center py-24 max-w-[480px] mx-auto">
                                ...
                                <h2 className="text-[1.5rem] font-bold mb-3 text-[var(--color-text-primary)]">Your cart is empty</h2>
                                <p className="text-[var(--color-text-secondary)] mb-8 leading-relaxed">Browse our events to find your best moments captured on camera.</p>
                                <button className="py-[14px] px-8 bg-[var(--color-brand-primary)] text-white border-none rounded-[var(--radius-full)] font-semibold cursor-pointer transition-all duration-200 ease-in-out hover:bg-[var(--color-brand-hover)] hover:-translate-y-px hover:shadow-[0_4px_12px_rgba(27,58,236,0.25)]" onClick={() => navigate('/')}>
                                    Start browsing
                                </button>
                            </div>
                        ...
                        <div className="grid grid-cols-1 max-lg:grid-cols-1 lg:grid-cols-[1fr_360px] gap-8 items-start">
                            <div className="cart-items-section">
                                    ...
                                    {cart.map((item) => (
                                        ...
                                        >
                                            <div className="w-[120px] max-sm:w-full h-[90px] max-sm:h-[180px] rounded-[var(--radius-md)] overflow-hidden flex-shrink-0 bg-[var(--ui-bg-subtle)]">
                                                <img src={item.photo.src} alt={item.photo.rider} className="w-full h-full object-cover" />
                                            </div>
                                            <div className="flex-1 flex max-sm:flex-col justify-between max-sm:gap-4">
                                                <div className="flex flex-col gap-1">
                                                    <div className="text-[0.75rem] font-bold text-[var(--color-text-secondary)] tracking-[0.05em]">#{item.photoId.toUpperCase()}</div>
                                                    <h3 className="text-[1rem] font-bold text-[var(--color-text-primary)] m-0">{item.photo.rider} / {item.photo.horse}</h3>
                                                    <div className="text-[0.8125rem] text-[var(--color-text-secondary)]">{item.photo.event}</div>
                                                    <div className="text-[0.75rem] text-[var(--color-text-secondary)] bg-[var(--ui-bg-subtle)] px-2 py-0.5 rounded-[4px] inline-block w-fit mt-1">{item.qualityLabel}</div>
                                                </div>
                                                <div className="flex flex-col max-sm:flex-row items-end max-sm:items-center justify-between" onClick={(e) => e.stopPropagation()}>
                                                    <div className="text-[1.125rem] font-bold text-[var(--color-text-primary)]">{item.price} SEK</div>
                                                    <button
                                ...
                                <button className="bg-none border-none flex items-center gap-2 text-[0.875rem] font-semibold text-[var(--color-text-secondary)] cursor-pointer py-2 px-0 transition-colors duration-200 ease-in-out hover:text-[var(--color-text-primary)]" onClick={() => fromPath ? navigate(decodeURIComponent(fromPath)) : navigate(-1)}>
                                    <ArrowLeft size={18} />
                                    Continue browsing
                                </button>
