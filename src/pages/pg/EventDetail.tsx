...
export const EventDetail: React.FC = () => {
    ...
    const renderContent = () => {
        return (
            <>
                ...
                <div
                    className={`bg-[var(--color-bg)] relative ${isExpanded ? 'flex-1 overflow-y-auto pt-0 pb-[100px] min-h-0' : 'pt-0'}`}
                    ref={scrollContainerRef}
                >
                    {/* Tabs Row (Inside scroll container for expanded autohide/stick behavior) */}
                    <div
                        className={`bg-white/95 backdrop-blur-[8px] border-b border-[var(--color-border)] ${isExpanded ? 'sticky top-0 z-20 shadow-[0_4px_12px_rgba(0,0,0,0.03)]' : ''}`}
                        style={undefined}
                    >
                        <div className="container flex gap-0 items-center">
                            ...
                            {isExpanded && (
                                <div className="ml-auto flex items-center gap-2.5">
                                    <img src={event!.logo} alt="" className="w-6 h-6 rounded-[4px] object-cover" />
                                    <span title={event!.title} className="text-[0.875rem] font-semibold text-[var(--color-text-primary)] max-w-[280px] overflow-hidden text-ellipsis whitespace-nowrap">
                                        {event!.title}
                                    </span>
                                </div>
                            )}
                                            ...
                                            <div className="pg-events-filter-right flex items-center">
                                                {selectedIds.size > 0 ? (
                                                    ...
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        {/* Edit Action */}
                                                        <button className={`${roundBtnBase} ${roundBtnLg}`} onClick={() => setIsPanelOpen(true)} title="Edit selection">
                                                            <Pencil size={18} />
                                                        </button>

                                                        {activeTab === 'uploads' || activeTab === 'archive' ? (
                                                            <>
                                                                <button className={`${roundBtnDelete} ${roundBtnLg}`} onClick={handleDeleteSelection} title="Delete selection">
                                                                    <Trash2 size={18} />
                                                                </button>
                                                                <button className={`${chipBtnPrimary} ${chipBtnLg} ml-2`} onClick={handlePublishSelection}>
                                                        ...
                                                        <button className={`${chipBtnGhost} text-secondary`} onClick={handleClearSelection}>
                                                            Clear
                                                        </button>
                                                    </div>
                                                ) : (
                                                    ...
                                                    <>
                                                        ...
                                                        <div className="flex items-center">
                                                            <div className="flex items-center gap-2 ml-auto pl-4 border-l border-black/10 shrink-0">
                                                                {isAllSelected ? (
                                                                    <button className={chipBtnGhost} onClick={() => setSelectedIds(new Set())}>
                                                                        <X size={14} />
                                                                        Clear
                                                                    </button>
                                                                ) : (
                                                                    <button className={chipBtnGhost} onClick={handleSelectAll}>
                                                                        Select all
                                                                    </button>
                                                                )}
                                                            </div>
                                        ...
                                        <div className="pg-photos-grid flex-1">
                                            {activeTab === 'uploads' && isDuplicatesFolder ? (
                                                ...
                                                <div className="pg-duplicates-list pb-[100px]">
                                                    {Array.from(duplicateGroups.entries()).map(([groupId, photos]) => {
                                                        ...
                                                        return (
                                                            <div key={groupId} id={`dup-group-${groupId}`} className="mb-8 bg-white rounded-xl p-5 shadow-[0_1px_3px_rgba(0,0,0,0.05)] border-2 border-transparent transition-all duration-500 ease-in-out">
                                                                <div className="pg-duplicate-header flex items-center justify-between mb-4">
                                                                    <div className="pg-dup-title font-semibold text-base flex items-center gap-2">
                                                                        Duplicate: {firstPhoto.fileName} <span className="text-[var(--color-text-secondary)] font-normal">({count} instances)</span>
                                                                    </div>
                                                                </div>
                                                                <div className="pg-duplicate-row flex gap-4 overflow-x-auto pb-2">
                                                                    {photos.map((photo) => {
                                                                        ...
                                                                        return (
                                                                            <div key={photo.id} className="w-[260px] shrink-0 relative">
                                                                                <div
                                                                                    className={`group relative cursor-pointer transition-[box-shadow] duration-200 ${isSelected ? 'pg-card-selected shadow-[0_0_0_2px_rgba(27,58,236,0.4)] z-[5] rounded-[var(--radius-md,8px)]' : ''}`}
                                                                                ...
                                                                                >
                                                                                    ...
                                                                                    <div
                                                                                        className={`absolute top-[10px] left-[10px] z-50 transition-opacity duration-200 group-hover:opacity-100 ${isSelected ? 'opacity-100' : 'opacity-0'}`}
                                                                                        onClick={(e) => { e.stopPropagation(); handleToggleSelect(photo, true); }}
                                                                                    >
                                                                                        <div className={`w-[18px] h-[18px] rounded-[4px] border flex items-center justify-center transition-all duration-200 ${isSelected ? 'bg-[var(--color-brand-primary)] border-[var(--color-brand-primary)]' : 'bg-white/90 border-white/60'}`}>
                                                    ...
                                                    {duplicateGroups.size === 0 && (
                                                        <div className="py-16 text-center text-[var(--color-text-secondary)] text-[0.875rem]">No duplicates found.</div>
                                                    )}
                                                </div>
                                            ) : (
                                            ...
                                            {activeTab === 'uploads' && !isDuplicatesFolder && displayedPhotos.length === 0 && (
                                                <div className="pg-empty-state">
                                                    <div className="pg-empty-icon"><ImageOff size={24} /></div>
                                                    <h3>{searchTerm ? 'No results found' : 'No photos here'}</h3>
                                                    <p>{searchTerm ? `No photos match "${searchTerm}"` : 'This folder is empty. Upload photos to get started.'}</p>
                                                </div>
                                            )}
                                            {activeTab === 'published' && displayedPhotos.length === 0 && (
                                                <div className="pg-empty-state">
                                                    <div className="pg-empty-icon"><Globe size={24} /></div>
                                                    <h3>{searchTerm ? 'No results found' : 'Nothing published yet'}</h3>
                                                    <p>{searchTerm ? `No published photos match "${searchTerm}"` : 'Publish photos from the Uploads tab to make them available for buyers.'}</p>
                                                </div>
                                            )}
                                            {activeTab === 'archive' && displayedPhotos.length === 0 && (
                                                <div className="pg-empty-state">
                                                    <div className="pg-empty-icon"><Archive size={24} /></div>
                                                    <h3>{searchTerm ? 'No results found' : 'Archive is empty'}</h3>
                                                    <p>{searchTerm ? `No archived photos match "${searchTerm}"` : 'Unpublished photos will appear here.'}</p>
                                                </div>
                                            )}
                                        </div>
    ...
    const chipBtnPrimary ="h-8 px-4 rounded-full bg-[var(--color-brand-primary)] text-white border-transparent text-[0.8125rem] font-semibold cursor-pointer transition-all duration-200 flex items-center gap-1.5 whitespace-nowrap shadow-[0_2px_6px_rgba(27,58,236,0.25)] hover:bg-[var(--color-brand-primary-hover)] hover:-translate-y-px";
    const chipBtnGhost = "h-8 px-3 rounded-full border-transparent bg-transparent text-[0.8125rem] font-semibold cursor-pointer transition-all duration-200 flex items-center gap-1.5 whitespace-nowrap hover:bg-[var(--ui-bg-subtle)]";
    const chipBtnLg = "!h-10 !px-5 !text-[0.875rem]";
    const roundBtnBase = "w-8 h-8 rounded-full border border-black/10 bg-white flex items-center justify-center cursor-pointer text-[var(--color-text-secondary)] transition-all duration-200 flex-shrink-0 hover:bg-[var(--ui-bg-subtle)] hover:text-[var(--color-text-primary)]";
    const roundBtnDelete = "w-8 h-8 rounded-full border border-[rgba(239,68,68,0.2)] bg-[var(--color-danger-tint)] flex items-center justify-center cursor-pointer text-[var(--color-danger)] transition-all duration-200 flex-shrink-0 hover:bg-[var(--color-danger-tint)] hover:border-[var(--color-danger)]";
    const roundBtnLg = "!w-10 !h-10";

    return (
        <div
            className={`pb-0 transition-all duration-[400ms] ease-[cubic-bezier(0.2,0,0.2,1)] ${isExpanded ? 'fixed inset-0 z-[2000] bg-white flex flex-col overflow-hidden' : ''}`}
        >
            <style>{`
                @keyframes modalPop {
                        ...
                        <ActionCluster>
                            ...
                            <div className="flex items-center gap-2">
                                <div className="inline-flex flex-col items-start justify-center gap-0 px-4 py-2 rounded-xl bg-[var(--ui-bg-subtle)] text-secondary whitespace-nowrap text-left">
                                    <span className="text-[0.6875rem] font-semibold leading-[1.2] opacity-80">Published</span>
                                    <span className="text-[0.875rem] font-bold leading-[1.2] text-primary">{event.publishedCount ?? 0}</span>
                                </div>
                                <div className="inline-flex flex-col items-start justify-center gap-0 px-4 py-2 rounded-xl bg-[var(--color-success-tint)] text-[var(--color-success)] whitespace-nowrap text-left">
                                    <span className="text-[0.6875rem] font-semibold leading-[1.2] opacity-80">Sales</span>
                                    <span className="text-[0.875rem] font-bold leading-[1.2]">{event.soldCount ?? 0}/{(event.photosCount ?? 40)}</span>
                                </div>
                                {isAdmin ? (
                                    <>
                                    ...
                                    <div className="inline-flex flex-col items-start justify-center gap-0 px-4 py-2 rounded-xl bg-[var(--color-revenue-bg)] text-[var(--color-revenue-text)] whitespace-nowrap text-left">
                                        <span className="text-[0.6875rem] font-semibold leading-[1.2] opacity-80">Earnings</span>
                                        <span className="text-[0.875rem] font-bold leading-[1.2] text-[var(--color-revenue-value)]">SEK {((event.soldCount ?? 0) * 450).toLocaleString().replace(/,/g, ' ')}</span>
                                    </div>
                                )}
                            </div>
            ...
            {/* Preview Modal */}
            {previewPhoto && (
                <div className="fixed inset-0 z-[3000] bg-black/95 flex items-center justify-center animate-[fadeIn_0.2s_ease]" onClick={() => setPreviewPhoto(null)}>
                    ...
                    <button className="absolute top-6 right-6 bg-white/10 text-white rounded-full w-11 h-11 flex items-center justify-center backdrop-blur-[10px] transition-[background] duration-200" onClick={() => setPreviewPhoto(null)}>
                        <X size={24} />
                    </button>
                </div>
            )}
                ...
                <div className="pg-modal-overlay" style={{ zIndex: 3000 }}>
                    <div className="pg-modal-card">
                        <div className="flex gap-4 items-start">
                            <div className={`pg-alert-icon ${(confirmModal.type === 'delete' || confirmModal.type === 'unpublish') ? 'danger' : 'info'}`}>
                                <AlertCircle size={24} />
                            </div>
                            <div className="flex-1">
                                ...
                                <div className="modal-footer-actions">
                                    <button className="modal-btn-cancel" onClick={() => setConfirmModal({ ...confirmModal, isOpen: false })}>Cancel</button>
                                    <button
                                        className={confirmModal.type === 'delete' || confirmModal.type === 'unpublish' ? 'modal-btn-danger' : 'modal-btn-save'}
            ...
            {showInfoModal && (
                <div className="auth-modal-overlay !z-[2100] !items-center" onClick={() => setShowInfoModal(false)}>
                    <div
                    ...
                    >
                        ...
                        <div className="modal-header-standard">
                            <h2 id="event-details-title" className="modal-title">Event details</h2>
                            <button
                            ...
                            <div className="flex flex-col gap-5">
                                ...
                                <div className="flex gap-4 items-center">
                                    <div className="w-20 h-20 shrink-0 rounded-xl overflow-hidden border border-black/[0.08]">
                                        <img src={event.logo} alt="" className="w-full h-full object-contain p-2" />
                                    </div>
                                    <div>
                                        <div className="event-info-title">{event.title}</div>
                                        <div className="text-[0.875rem] text-[var(--color-text-secondary)]">{event.dateRange}</div>
                                    </div>
                                </div>
                                ...
                                <div>
                                    ...
                                    <div className="event-info-value font-medium">EquiSport Events AB</div>
                                </div>
