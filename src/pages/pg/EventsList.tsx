        ...
        <div className="pg-events-container" onClick={() => setActiveMenuId(null)}>
            ...
            {(isAdmin || view === 'upcoming') ? (
                <div className="pg-events-list upcoming-list">
                    {activeList.length === 0 ? (
                        <div className="pg-empty-state">
                            <div className="pg-empty-icon"><CalendarX2 size={24} /></div>
                            <h3>No events found</h3>
                            <p>Try adjusting your filters or search to find matching events.</p>
                        </div>
                    ) : (
                        activeList.map((event, index) => {
                            ...
                            return (
                                <div
                                    key={event.id}
                                    className={`pg-event-row-grid upcoming-event relative ${isApplied ? 'applied' : ''} ${isAdmin ? 'clickable-row' : ''}`}
                                        ...
                                        <div className="pg-applied-action-group">
                                            {isAdmin ? (
                                                <div className="pg-admin-card-actions">
                                                    ...
                                                    <div className="pg-more-menu-container">
                                                        ...
                                                        {activeMenuId === event.id && (
                                                            <div className="dropdown-menu absolute top-[calc(100%+8px)] right-0 w-max min-w-[180px] z-[1000]" onClick={(e) => e.stopPropagation()}>
                                                                {view === 'archived' ? (
                                                                    <>
                                                                        <button className="dropdown-item" onClick={() => handleNavigateToEvent(event.id)}>
                                                                            <Search size={14} />
                                                                            View Dashboard
                                                                        </button>
                                                                        <button className="dropdown-item" onClick={() => { console.log('Make active', event.id); setActiveMenuId(null); }}>
                                                                            <CheckCircle size={14} />
                                                                            Make active
                                                                        </button>
                                                ...
                                                <>
                                                    ...
                                                    <FilterChip
                                                        label="Withdraw"
                                                        onClick={(e?: any) => handleCancelAppClick(event, e)}
                                                    ...
                                                    {(() => {
                                                        ...
                                                        return (
                                                            <>
                                                                ...
                                                                <FilterChip
                                                                    label="Apply"
                                                                    onClick={(e?: any) => { e?.stopPropagation?.(); setApplyingEvent(event); setIsApplyModalOpen(true); }}
                                                                />
                                                            </>
                                                        );
                                                    })()}
                ...
                <div className="pg-events-grid">
                    {myEvents.length === 0 ? (
                        <div className="pg-empty-state">
                            <div className="pg-empty-icon"><CalendarPlus size={24} /></div>
                            <h3>No events yet</h3>
                ...
                <div className="pg-modal-overlay">
                    <div className="pg-modal-card">
                        <div className="flex gap-4 items-start">
                            ...
                            <div className="flex-1">
                                <h3 className="mt-0 text-[1.125rem] font-bold mb-2 text-primary">
                                    ...
                                    {confirmModal.type === 'cancel_application' && 'Withdraw application?'}
                                </h3>
                                <p className="mb-4 text-[0.875rem] leading-[1.5] text-secondary">
                                    ...
                                    {confirmModal.type === 'cancel_application' && (
                                        <>You are withdrawing your application for <strong>{confirmModal.event?.title}</strong>.</>
                                    )}
                                </p>
                                {confirmModal.type === 'cancel_application' && (
                                    <div className="mb-6">
                        ...
                        <div className="modal-footer-actions">
                            <button className="modal-btn-cancel" onClick={() => { setConfirmModal(null); setWithdrawReason(''); }}>
                                Cancel
                            </button>
                            <button
                                className="modal-btn-danger"
                                disabled={confirmModal.type === 'cancel_application' && !withdrawReason.trim()}
                                onClick={() => {
