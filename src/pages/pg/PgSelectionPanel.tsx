...
import { X, Clock, Trash2, MoreVertical, RotateCcw, Save, Slash, ChevronDown, Plus, Check, Pencil, Info, ChevronRight, HelpCircle, ChevronLeft } from 'lucide-react';
...
export const PgSelectionPanel: React.FC<PgSelectionPanelProps> = ({
        ...
        if (selectedPhotos.length > 0) {
            ...
            if (selectedPhotos.length > 1) {
                ...
                setOriginalState({
                    ...
                    priceWeb: pWeb, priceHigh: pHigh, priceCommercial: pComm, priceBundle: bundle
                });
                setViewMode('edit');
            } else {
                // Single
    ...
    const isDirty = useMemo(() => {
        ...
        const priceDirty = priceBundle !== originalState.priceBundle ||
    ...
    return (
        <>
            <div className={`pg-selection-panel ${isOpen ? 'is-open' : ''}`}>
                                    ...
                                    {viewMode === 'edit' ? (
                                        ...
                                        <div className="mt-4 p-4 bg-[var(--ui-bg-subtle)] rounded-lg">
                                            <div className="flex items-start justify-between mb-2">
                                                <div style={{ fontSize: '0.6875rem', fontWeight: 600, color: 'var(--color-text-secondary)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Generic photo</div>
                                                <button
                                ...
                                <div>
                                    ...
                                    <div className="pg-price-bundle-list flex flex-col gap-3 mb-6">
                                        {(Object.keys(BUNDLES) as Array<keyof typeof BUNDLES>).map(key => {
                                            ...
                                            const colors = {
                                                basic: { dot: 'var(--color-text-secondary)', bg: 'var(--ui-bg-subtle)', border: 'var(--color-border)' },
                                            ...
                                            return (
                                                ...
                                                >
                                                    {/* Left Accent Bar */}
                                                    <div className="absolute left-0 top-0 bottom-0 w-1" style={{ background: c.dot }} />
                                                    <div
                                                        className="w-4 h-4 rounded-full bg-white mt-0.5 flex items-center justify-center shrink-0"
                                                        style={{ border: `1px solid ${isSelected ? 'var(--color-brand-primary)' : 'var(--color-border)'}` }}
                                                    >
                                                        {isSelected && <div className="w-2 h-2 rounded-full bg-[var(--color-brand-primary)]" />}
                                                    </div>
                                                    <div className="flex-1">
                                                        <div className="flex justify-between items-baseline mb-0.5">
                                                            <div className="text-[0.8125rem] font-medium text-primary">{b.label}</div>
                                                            <div className="text-[0.8125rem] font-normal text-secondary">{info.primaryPrice} SEK</div>
                                                        </div>
                                                        <div className="text-[0.75rem] text-[var(--color-text-secondary)] mb-0.5">{info.subtitle}</div>
                                                        <div className="text-[0.75rem] text-secondary leading-[1.4]">{info.support}</div>
                                                    </div>
                                                </div>
                                            );
                                        })}
                                        ...
                                        >
                                            <div className="flex items-start gap-3">
                                                <div
                                                    ...
                                                    style={{ border: `1px solid ${priceBundle === 'custom' ? '#facc15' : 'var(--color-border)'}` }}
                                                >
                                                    {priceBundle === 'custom' && <div className="w-2 h-2 rounded-full bg-[#facc15]" />}
                                                </div>
                                                <div className="flex-1">
                                                    <div className="text-[0.8125rem] font-medium text-primary">Custom</div>
                                                    {priceBundle !== 'custom' && (
                                                        <div className="text-[0.75rem] text-secondary">Set your own price for all levels</div>
                                                    )}
                                                </div>
                                            </div>
                                            {/* Inputs for Custom */}
                                            {priceBundle === 'custom' && (
                                                <div className="pg-form-group mt-3 ml-7">
                                                    <div className="text-[var(--fs-xs)] text-secondary mb-1">Price (SEK)</div>
                                                    <input
                ...
                {/* Footer */}
                <div className={`pg-panel-footer ${isDirty || editFromSummary ? 'dirty-state' : ''}`}>
                ...
                {isMoveModalOpen && (
                    <div className="pg-panel-confirm-overlay" onClick={() => setIsMoveModalOpen(false)}>
                        <div className="pg-panel-confirm-card" onClick={(e) => e.stopPropagation()}>
                            <h3>Move to other folder</h3>
                            <div className="pg-modal-body mb-6">
                                {!isCreatingBatch ? (
                                    <>
                                        <div className="text-[0.8125rem] font-semibold text-secondary mb-2">Select Folder</div>
                                                ...
                                                <div className="pg-custom-select-list" onClick={(e) => e.stopPropagation()}>
                                                    {BATCHES.map(b => {
                                                        ...
                                                        return (
                                                            ...
                                                            >
                                                                {b}
                                                                {isDisabled && <span className="text-[var(--fs-xs)] text-[var(--color-text-secondary)]">Current folder</span>}
                                                            </div>
                                                        );
                                                    })}
                                                    <div
                                    ...
                                    <div className="pg-form-group">
                                        <div className="text-[0.8125rem] font-semibold text-secondary mb-2">New Folder Name</div>
                                        <div className="flex gap-2">
                                            <input
                                                className="pg-panel-input"
                            ...
                            <div className="pg-panel-confirm-actions">
                                <button className="pg-panel-btn secondary" onClick={() => setIsMoveModalOpen(false)}>Cancel</button>
                                <button
                    ...
                    <div className="pg-panel-confirm-overlay">
                        <div className="pg-panel-confirm-card">
                            ...
                            <p>
                                ...
                                {confirmModal.type === 'close' && 'You have unsaved changes. Are you sure you want to discard them?'}
                                {confirmModal.type === 'publish' && 'This will make the photo available for buyers.'}
                                {confirmModal.type === 'unpublish' && 'This will move the photo to the Archive tab.'}
                            </p>
                            <div className="pg-panel-confirm-actions">
                                <button className="pg-panel-btn secondary" onClick={() => setConfirmModal({ ...confirmModal, isOpen: false })}>
                                    Cancel
                                </button>
                                {confirmModal.type === 'delete' && (
                                    <button className="pg-panel-btn destructive" onClick={confirmDelete}>
                                        Delete
                                    </button>
                                )}
                                {confirmModal.type === 'refresh' && (
                                    <button className="pg-panel-btn primary" onClick={confirmRefresh}>
                                        Refresh
                                    </button>
                                )}
                                {confirmModal.type === 'close' && (
                                    <button className="pg-panel-btn destructive" onClick={confirmCloseDiscard}>
                                        Discard
                                    </button>
                                )}
                                {confirmModal.type === 'publish' && (
                                    <button className="pg-panel-btn primary" onClick={confirmPublish}>
                                        {isArchived ? "Republish" : "Publish"}
                                    </button>
                                )}
                                {confirmModal.type === 'unpublish' && (
                                    <button className="pg-panel-btn destructive" onClick={confirmUnpublish}>
                                        Unpublish
                                    </button>
                                )}
                            </div>
                ...
                {isInfoOpen && (
                    <div className="absolute inset-0 bg-[var(--color-surface)] z-[2000] flex flex-col [animation:fadeIn_0.2s_ease-out]">
                        ...
                        <div className="pg-panel-scroll-area p-6">
                            {/* Quality Info */}
                            <div className="mb-6">
                                <div className="text-[0.875rem] font-semibold text-primary mb-4">Quality info</div>
                                <div className="flex flex-col gap-4">
                                    <div>
                                        <div className="text-[0.875rem] font-medium text-primary mb-0.5">Web quality</div>
                                        <div className="text-[0.8125rem] text-secondary">Best for screen and social media.</div>
                                    </div>
                                    <div>
                                        <div className="text-[0.875rem] font-medium text-primary mb-0.5">High quality</div>
                                        <div className="text-[0.8125rem] text-secondary">Best for print and large displays.</div>
                                    </div>
                                    <div>
                                        <div className="text-[0.875rem] font-medium text-primary mb-0.5">Commercial use</div>
                                        <div className="text-[0.8125rem] text-secondary">Best for print and large displays.</div>
                                    </div>
                            ...
                            {/* Earnings Split */}
                            <div className="mb-6">
                                <div className="text-[0.875rem] font-semibold text-primary mb-4">Earnings split</div>
                            ...
                            <div className="flex flex-col gap-3">
                                {[
                                    { label: 'Terms of Service', icon: <ChevronRight size={15} color="var(--color-border)" /> },
                                    { label: 'Privacy Policy', icon: <ChevronRight size={15} color="var(--color-border)" /> },
                                    { label: 'FAQ', icon: <HelpCircle size={15} color="var(--color-border)" /> },
                                ].map(({ label, icon }) => (
                                    <a
                                        key={label}
                                        href="#"
...
const Label: React.FC<{ text: string }> = ({ text }) => (
    <div style={{ fontSize: '0.75rem', fontWeight: 600, color: 'var(--color-text-secondary)', marginBottom: 6 }}>{text}</div>
);
