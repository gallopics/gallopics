...
import {
    LayoutGrid, CreditCard, Settings, DollarSign,
    FileText, ChevronLeft, ChevronRight, Palette, Users
} from 'lucide-react';
            ...
            <div className="flex flex-1">
                ...
                >
                    ...
                    <div
                        ...
                        style={{ height: 36, justifyContent: collapsed ? 'center' : 'space-between', padding: collapsed ? 0 : '0 8px 0 16px' }}
                    >
                        <span
                    ...
                    <nav className="flex flex-col gap-0.5 flex-1 overflow-y-auto overflow-x-hidden px-[15px]">
                        ...
                        <SidebarNavItem to={`${basePath}/events`} icon={<LayoutGrid size={18} />} label="Events" collapsed={collapsed} onNavigate={handleNavClick} />
                        {isAdmin && (
                            <SidebarNavItem to={`${basePath}/photographers`} icon={<Users size={18} />} label="Photographers" collapsed={collapsed} onNavigate={handleNavClick} />
                        )}

                        <SidebarNavItem to={`${basePath}/sold`} icon={<DollarSign size={18} />} label="Sales" collapsed={collapsed} onNavigate={handleNavClick} />
                        <SidebarNavItem to={`${basePath}/receipts`} icon={<FileText size={18} />} label="Receipts" collapsed={collapsed} onNavigate={handleNavClick} />

                        {/* Divider */}
                        <div className="h-px bg-[var(--color-border)] mx-3 my-3" />

                        <SidebarNavItem to={`${basePath}/billing`} icon={<CreditCard size={18} />} label="Billing details" collapsed={collapsed} onNavigate={handleNavClick} />
                        <SidebarNavItem to={`${basePath}/settings`} icon={<Settings size={18} />} label="Settings" collapsed={collapsed} onNavigate={handleNavClick} />
