...
const MOCK_PHOTOGRAPHERS = [
  ...
  subEvents: [
    {
      id: `e${i}-1`,
      name: `${p.city} Horse Show`,
      published: Math.floor(p.published * 0.6),
      sold: `${Math.floor(p.sold * 0.6)}/${Math.floor(p.totalSold * 0.6)}`,
      revenue: Math.floor(p.revenue * 0.6),
    },
    ...(p.events > 1
      ? [
          {
            id: `e${i}-2`,
            name: `Summer Cup ${p.city}`,
            published: Math.floor(p.published * 0.4),
            sold: `${Math.floor(p.sold * 0.4)}/${Math.floor(p.totalSold * 0.4)}`,
            revenue: Math.floor(p.revenue * 0.4),
          },
        ]
      : []),
  ],
...
export const Photographers: React.FC = () => {
  ...
  const searchOptions = useMemo<ScopedSearchOption[]>(() => {
    const pNames = MOCK_PHOTOGRAPHERS.map(p => ({
      label: p.name,
      value: p.name,
      type: 'rider' as const,
    }));
    const eNames = MOCK_PHOTOGRAPHERS.flatMap(p => p.subEvents).map(e => ({
      label: e.name,
      value: e.name,
      type: 'horse' as const,
    }));
                  ...
                  {filteredPhotographers.map(p => (
                    <React.Fragment key={p.id}>
                      <tr>
                          ...
                          <div className="pg-user-cell">
                            <img
                              ...
                              onError={e => {
                                ...
                                ).parentElement?.classList.add('broken-avatar'); // Hook for CSS if needed
                              }}
                              alt=""
                            ...
                            <div className="pg-user-info-stack">
                              <Link
                                ...
                                className="pg-user-link"
                              >
                                {p.name}
                              </Link>
                              <div className="pg-user-city">
                                {(p as any).city}
                              </div>
                        ...
                        <td className="col-revenue text-right">
                          {p.revenue.toLocaleString().replace(/,/g, ' ')}
                        </td>
                        <td className="col-published text-right">
                          {p.published}
                        </td>
                        <td className="col-sold text-right">
                          {p.sold}/{p.totalSold}
                        </td>
                                  ...
                                  {p.subEvents.map(e => (
                                    <tr key={e.id}>
                                      <td>
                                        <Link
                                          to="/admin/events/1"
                                          className="pg-user-link text-[0.8125rem]"
                                        >
                                          {e.name}
                                        </Link>
                                      </td>
