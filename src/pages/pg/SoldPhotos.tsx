...
import {
  ScopedSearchBar,
  type ScopedSearchOption,
} from '../../components/ScopedSearchBar';
...
export const SoldPhotos: React.FC = () => {
  ...
  const filteredPhotos = useMemo(() => {
    ...
    return allSoldPhotos.filter(p => {
      ...
      const matchBundle =
        selectedBundleId === 'all' || mockBundle === selectedBundleId;
      ...
      if (selectedPeriod !== 'all') {
        ...
        if (selectedPeriod === 'this_month')
          ...
          matchPeriod = d.getFullYear() === now.getFullYear() - 1;
      }
  ...
  const searchOptions = useMemo<ScopedSearchOption[]>(() => {
    const ids = Array.from(
      new Set(
        allSoldPhotos.map(
          p => p.photoCode || `#M-0-${p.id.slice(0, 8).toUpperCase()}`,
        ),
      ),
  ...
  const mapToUiPhoto = (photo: Photo) => {
    ...
    return {
      ...
      time: purchaseDate.toLocaleTimeString('en-GB', {
        hour: '2-digit',
        minute: '2-digit',
      }),
  ...
  return (
    <div className="pg-sold-photos">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="w-[160px]">
                      <ModernDropdown
                        ...
                        variant="pill"
                        icon={
                          bundleOptions.find(o => o.value === selectedBundleId)
                            ?.icon
                        }
                      />
                    </div>
                      ...
                      <>
                        <div className="pg-badge-item earnings stacked">
                          ...
                          <span className="value">
                            SEK {REVENUE.toLocaleString().replace(/,/g, ' ')}
                          </span>
                        </div>
                        <div className="pg-badge-item earnings stacked pg-badge-gallop">
                          ...
                          <span className="value">
                            SEK{' '}
                            {(REVENUE * 10).toLocaleString().replace(/,/g, ' ')}
                          </span>
                        </div>
                      </>
                    ) : (
              ...
              <div className="pg-trend-content">
                <div className="pg-trend-chart">
                  {graphData.dataPoints.map((point, i) => {
                    ...
                    const showLabel =
                      selectedPeriod === 'this_month' ? i % 5 === 0 : true;
                    const label =
                    ...
                    return (
                      ...
                      >
                        <div className="pg-chart-bar-total">
                          {Math.round(total)}
                        </div>
                        <div
                          className="pg-chart-bar"
                          style={{ height: `${total}%` }}
                        >
                          <div
                            className="pg-bar-segment bundle-custom"
                            style={{
                              height: `${(point.custom / total) * 100}%`,
                            }}
                          />
                          <div
                            className="pg-bar-segment bundle-premium"
                            style={{
                              height: `${(point.premium / total) * 100}%`,
                            }}
                          />
                          <div
                            className="pg-bar-segment bundle-standard"
                            style={{
                              height: `${(point.standard / total) * 100}%`,
                            }}
                          />
                          <div
                            className="pg-bar-segment bundle-basic"
                            style={{
                              height: `${(point.basic / total) * 100}%`,
                            }}
                          />
                        </div>
                        {showLabel && (
                          <span className="pg-chart-label">{label}</span>
                        )}

                        {isHovered && (
                          <div
                            className="pg-chart-tooltip"
                            style={{
                              bottom: '100%',
                              left:
                                i > graphData.dataPoints.length / 2
                                  ? 'auto'
                                  : '50%',
                              right:
                                i > graphData.dataPoints.length / 2
                              ...
                              {isAdmin ? (
                                <>
                                  <div className="tooltip-row mt-1">
                                    ...
                                    <div className="value text-success-dark">
                                      SEK{' '}
                                      {Math.round(total * 450).toLocaleString()}
                                    </div>
                                  </div>
                                  <div className="tooltip-row mt-0.5">
                                    <div className="label">Gallop Earnings</div>
                                    <div className="value text-gallop">
                                      SEK{' '}
                                      {Math.round(
                                        total * 4500,
                                      ).toLocaleString()}
                      ...
                      <PhotoCard
                        ...
                        onRemove={() => {
                          setConfirmModal({
                            ...
                            photoCode:
                              photo.photoCode ||
                              `#M-0-${photo.id.slice(0, 8).toUpperCase()}`,
                          });
                        }}
                        pgMeta={{
                          ...
                          <tr key={photo.id}>
                            <td>
                              <img
                                ...
                                alt=""
                                onClick={() =>
                                  setPreviewPhoto(mapToUiPhoto(photo))
                                }
                              />
                            </td>
                            <td title={photoIdDisplay}>
                              <div className="truncate-cell-content font-semibold text-primary">
                                {photoIdDisplay}
                              </div>
                              {photo.fileName && (
                                <div className="truncate-cell-content text-[var(--fs-xs)] text-secondary">
                                  {photo.fileName}
                                </div>
                              )}
                            </td>
                            <td
                              title={`${photo.uploadDate} ${photo.timestamp || '14:20'}`}
                            >
                              <div className="truncate-cell-content font-medium">
                                {photo.uploadDate}
                              </div>
                              <div className="truncate-cell-content text-[var(--fs-xs)] text-secondary">
                                {photo.timestamp || '14:20'}
                              </div>
                            </td>
                            <td title={eventTitle}>
                            ...
                            <td title={clientEmail}>
                              <div className="truncate-cell-content text-secondary">
                                {clientEmail}
                              </div>
                            </td>
                            <td title={getBundleLabel(photo.id)}>
                              <div className="pg-bundle-tag">
                                <div
                                  className="pg-bundle-dot"
                                  style={{
                                    background: getBundleColor(photo.id),
                                  }}
                                />
                                <span className="truncate-cell-content">
                                  {getBundleLabel(photo.id)}
                                </span>
                              </div>
                            </td>
                            <td className="text-center">
                              <div className="inline-flex justify-center w-full">
                                <div className="pg-sold-badge">
                                  {photo.soldCount || 1}
                                </div>
                              </div>
                            </td>
                            <td
                              title={`https://gallopics.com/dl/${photo.photoCode || photo.id.slice(0, 8)}`}
                            >
                              <a
                                href="#"
                                className="pg-download-link"
      ...
      {previewPhoto && (
        <div
          className="fixed inset-0 z-[3000] bg-black/95 flex items-center justify-center [animation:fadeIn_0.2s_ease]"
        ...
        >
          ...
          />
          <button
            className="absolute top-6 right-6 bg-white/10 text-white rounded-full w-11 h-11 border-none cursor-pointer flex items-center justify-center backdrop-blur-[10px] transition-[background] duration-200"
            onClick={() => setPreviewPhoto(null)}
          >
            <X size={24} />
          </button>
            ...
            <div className="flex gap-4 items-start">
              ...
              <div className="flex-1">
                ...
                <p className="m-0 mb-6 text-secondary text-[0.875rem] leading-[1.5]">
                  This will move photo <strong>{confirmModal.photoCode}</strong>{' '}
                  to the Archive tab. You can undo this action later from the
                  event details page.
                </p>
              </div>
            </div>
