                ...
                <div className="w-full aspect-video overflow-hidden relative rounded-xl bg-[var(--ui-bg-subtle)]">
                    ...
                    {!hasCover && (
                        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                            <span className="text-[0.8125rem] font-medium text-[var(--color-text-secondary)]">Add cover photo</span>
                        </div>
                    )}
                ...
                <div className="pt-4 flex flex-col">
                    <div className="flex items-center gap-4 mb-4">
                        <div className="w-[52px] h-[52px] rounded-xl overflow-hidden bg-white border border-black/[0.06] flex-shrink-0">
                            <img src={event.logo} alt="" className="w-full h-full object-contain p-0.5" />
                        </div>
                        <div className="flex flex-col justify-center flex-1">
                            <div className="flex items-center gap-1 mb-0.5">
                                <span className="text-[0.75rem] leading-none">🇸🇪</span>
                                <span className="text-[0.75rem] font-semibold text-[var(--color-text-secondary)] uppercase tracking-[0.02em] leading-[1.5]">{event.city}</span>
                            </div>
                            <h3 className="text-[0.8125rem] font-bold text-[var(--color-text-primary)] my-1 leading-[1.5]">{event.title}</h3>
                            <span className="text-[0.75rem] text-[var(--color-text-secondary)] mt-0.5 leading-[1.5]">{event.dateRange}</span>
                        </div>
                        ...
                        <div className="relative flex items-center ml-auto">
                            ...
                            {isMenuOpen && (
                                ...
                                >
                                    <button className="dropdown-item" onClick={() => { setIsMenuOpen(false); navigate(`${basePath}/events/${event.id}`); }}>
                                        <ImageIcon size={14} />
                                        Manage photos
                                    </button>
                                    <button className="dropdown-item" onClick={() => { setIsMenuOpen(false); onEdit?.(event); }}>
                                        <Edit2 size={14} />
                                        Edit Event
                                    </button>
                                </div>
                    ...
                    <div className="flex justify-end items-center">
                        <div className="flex items-center gap-2">
                            <div className="inline-flex flex-col items-start justify-center gap-0 px-3.5 py-1.5 rounded-lg bg-[var(--ui-bg-subtle)] text-[var(--color-text-secondary)] text-[0.75rem] font-semibold whitespace-nowrap text-left">
                                <span className="text-[0.625rem] font-semibold leading-[1.2] opacity-80">Published</span>
                                <span className="text-[0.8125rem] font-bold leading-[1.2]">{event.publishedCount ?? 0}</span>
                            </div>
                            <div className="inline-flex flex-col items-start justify-center gap-0 px-3.5 py-1.5 rounded-lg bg-[var(--color-success-tint)] text-[var(--color-success)] text-[0.75rem] font-semibold whitespace-nowrap text-left">
                                <span className="text-[0.625rem] font-semibold leading-[1.2] opacity-80">Sales</span>
                                <span className="text-[0.8125rem] font-bold leading-[1.2]">{event.soldCount ?? 0}/{(event.photosCount ?? 40)}</span>
                            </div>
                            <div className="inline-flex flex-col items-start justify-center gap-0 px-3.5 py-1.5 rounded-lg bg-[var(--color-revenue-bg)] text-[var(--color-revenue-text)] text-[0.75rem] font-semibold whitespace-nowrap text-left">
                                <span className="text-[0.625rem] font-semibold leading-[1.2] opacity-80">Earnings</span>
                                <span className="text-[0.8125rem] font-bold leading-[1.2] text-[var(--color-revenue-value)]">SEK {((event.soldCount ?? 0) * 450).toLocaleString().replace(/,/g, ' ')}</span>
                            </div>
                        </div>
                    </div>
                </div>
